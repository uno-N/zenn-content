...
emoji: "🔐"
type: "tech"
topics: ["個人開発", "devops", "sentry", "react", "セキュリティ"]
published: true
---
...
パッケージをインストールしたあと、定期的に実行してください。

```bash
npm audit
```

今回私のプロジェクトでは **Viteに高深刻度の脆弱性**が検出されました。
...
これだけです。あとはSentryの管理画面のIssuesを見れば、エラーの内容・発生場所・ブラウザ情報が全部わかります。
...
*この記事はReact + Viteを使った個人開発アプリ「ねぃろ」の開発日記シリーズです。*
*[#1 Suno API](/articles/dev-diary-01-suno-api) / [#2 React環境構築](/articles/dev-diary-02-react-setup) / [#3 Git・GitHub](/articles/dev-diary-03-git-github) / [#4 音楽API比較](/articles/dev-diary-04-music-api) / [#5 Vite移行](/articles/dev-diary-05-vite-ui)*
